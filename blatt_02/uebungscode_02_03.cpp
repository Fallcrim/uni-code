...
void alter_von_alan_turing_erraten() {
  ...
  while (Rateversuch != alter) {
    drucke_text("Wie alt wäre Alan Turing dieses Jahr geworden?");
    Rateversuch = lies_ganzzahl();
    if (Rateversuch < alter) {
      drucke_text("Alan Turing wäre älter.");
    } else {
      if (Rateversuch > alter) {
        drucke_text("Alan Turing wäre jünger.");
      }
      
    }
  }
  drucke_text("Richtig. Alan Turing wäre heute 113 Jahre alt.");
}
